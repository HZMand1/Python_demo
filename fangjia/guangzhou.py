...
class lianjia():
    def __init__(self):
        ...
        self.pattern = re.compile(
            '<div class="info clear">.*?href="(.*?)" target="_blank">(.*?)</a>.*?class="houseInfo"><span class="houseIcon"></span>(.*?)</div></div>.*?class="totalPrice"><span>(.*?)</span>万.*?class="unitPrice".*?<span>(.*?)</span></div></div></div>')
        self.house_num_pattern = re.compile('共找到<span> (.*?) </span>套(.*?)二手房')
        self.area_dic = {'越秀': 'yuexiu',
                         '荔湾': 'liwan'
                         }
    ...
    def get_content(self, info, area):
        ...
        if info:
            info = list(info[0])
            info_dic['url'] = info[0].strip()
            info_dic['title'] = info[1].strip()
            info_dic['totalPrice'] = info[3].strip() + '万'
            info_dic['unitPrice'] = info[4].strip()
    ...
    def run(self):
        ...
        for area in self.area_dic.keys():
            print('>>>> 正在保存%s的二手房信息>>>\n' % area)
            url = 'https://gz.lianjia.com/ershoufang/%s/' % self.area_dic[area]
            ...
            # 创建表头数据
            head = ['标题', '总价', '单价', '户型', '大小', '朝向', '装修程度', '楼层位置', '修建时间', '楼房结构', '房源URL']
            for h in range(len(head)):
                sheet.write(0, h, head[h])
            i = 1
            for page in tqdm(range(total_page)):
                url = 'https://gz.lianjia.com/ershoufang/%s/pg%s/' % (self.area_dic[area], str(page + 1))
                ...
                for info in infos:
                    ...
                    if info_dic:
                        ...
                        sheet.write(i, 10, info_dic['url'])
                        i += 1
                        print('>>>> 链家二手房数据已保存❗----------%s' % area)

        workbook.save('D:\\guangzhou.xlsx')
